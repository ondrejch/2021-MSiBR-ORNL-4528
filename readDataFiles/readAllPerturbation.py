...
class plotGraph:
    def __init__(self, xArray, yArray, yErrorArray, lbl=None, clr=None):
        ...
        plt.plot(self.xArray, self.yArray, '.', lw=1, color=self.clr)
        plt.plot(self.xArray, self.yLine, lw=1, color=self.clr, label=self.lbl)
...
def main():
    exceptions = ['runAll.py', 'readAllPerturbation.py', 'keff.png', 'reactivity.png','conv.png']
    ...
    v1Order = np.array([-100,-50,0,100,50])


    plotGraph(v1Order, keffArray[0], kErrorArray[0], 'All',         'black')
    plotGraph(v1Order, keffArray[1], kErrorArray[1], 'Blanket',     'blue')
    plotGraph(v1Order, keffArray[2], kErrorArray[2], 'Fuel',        'red')
    plotGraph(v1Order, keffArray[3], kErrorArray[3], 'Graphite',    'green')
    ...
    plt.grid()
    # plt.title('k-eff')
    plt.xlabel('Temperature [ΔK]')
    plt.ylabel(r'k$_{eff}$')
    plt.savefig('keff.png')
    plt.show()

    reactivityArray = reactivity(keffArray, kErrorArray)
    reactivityArrayError = reactivityArray[1]
    reactivityArray = reactivityArray[0]
    plotGraph(v1Order, reactivityArray[0], reactivityArrayError[0], 'All', 'black')
    plotGraph(v1Order, reactivityArray[1], reactivityArrayError[1], 'Blanket', 'blue')
    plotGraph(v1Order, reactivityArray[2], reactivityArrayError[2], 'Fuel', 'red')
    plotGraph(v1Order, reactivityArray[3], reactivityArrayError[3], 'Graphite', 'green')
